...
contract NoReturnToken {
    ...
    // ⚠ Void returns throughout, on purpose: SafeERC20 must tolerate them, bare IERC20 must not.
...
contract OpenPonsDistributorTest is Test {
    MockEscrow escrow;
    MockPoolManager pm;
    NoReturnToken usdg;
    NoReturnToken aapl;
    ...
    function setUp() public {
        ...
        d = new OpenPonsDistributor(
            IPonsFeeEscrow(address(escrow)), address(gv), PID, ops, 9_000,
            IPoolManager(address(pm)), address(usdg), 200, address(0), 0 // 2% max slippage
        );
        _seedPool();
    }
    ...
    function test_nativeFeesReachTheProject() public {
        vm.deal(address(this), 10 ether);
        escrow.credit{value: 1 ether}(address(d));
        d.harvest();
        assertEq(gv.balances(PID, address(0)), 0.9 ether);
        assertEq(ops.balance, 0.1 ether);
        assertEq(address(d).balance, 0, "nothing may rest in the distributor");
    }

    function test_usdgFeesReachTheProject_andNativeStaysZero() public {
        ...
        assertEq(gv.balances(PID, address(usdg)), 900e6);
    ...
    function test_harvestingNothingIsQuietAndCheap() public {
        d.harvest();
        d.harvestToken(address(usdg));
        assertEq(gv.balances(PID, address(0)), 0);
    }

    function test_roundingFavoursTheProject() public {
        vm.deal(address(this), 1 ether);
        escrow.credit{value: 9_999}(address(d));
        d.harvest();
        assertEq(ops.balance, 999);
        assertEq(gv.balances(PID, address(0)), 9_000, "the dust went to the project");
    }
    ...
    function test_aHarvestedStockIsHeldNotPushed() public {
        ...
        assertEq(aapl.balanceOf(address(gv)), 0, "a stock must NEVER reach a vault - no bridge takes it");
    }
    ...
    function test_sellingAStockPaysTheProjectInUsdg() public {
        aapl.mint(address(escrow), 10e18);
        escrow.creditToken(address(d), address(aapl), 10e18);
        d.harvestToken(address(aapl));
    ...
    function test_thereIsNoWayToChangeTheDestination() public view {
        assertEq(d.gitVault(), address(gv));
        assertEq(d.projectId(), PID);
        assertEq(d.projectBps(), 9_000);
    ...
    function test_hundredPercentToProjectIsAllowed() public {
        OpenPonsDistributor full = new OpenPonsDistributor(
            IPonsFeeEscrow(address(escrow)), address(gv), PID, address(0), 10_000,
            IPoolManager(address(pm)), address(usdg), 200, address(0), 0
        );
        vm.deal(address(this), 1 ether);
        escrow.credit{value: 1 ether}(address(full));
        full.harvest();
        assertEq(gv.balances(PID, address(0)), 1 ether);
    }

    function test_aSplitOverOneHundredPercentIsRefused() public {
        vm.expectRevert(OpenPonsDistributor.BadSplit.selector);
        new OpenPonsDistributor(IPonsFeeEscrow(address(escrow)), address(gv), PID, ops, 10_001,
            IPoolManager(address(pm)), address(usdg), 200, address(0), 0);
    }

    function test_projectLegLandsInTheVaultUnderItsOwnProjectId() public {
        // ⭐ A second distributor, 70/30, same vault: attribution rides in the call, not the address.
        bytes32 pid2 = bytes32(bytes("foundry-rs/foundry"));
        OpenPonsDistributor d2 = new OpenPonsDistributor(
            IPonsFeeEscrow(address(escrow)), address(gv), pid2, ops, 7_000,
            IPoolManager(address(pm)), address(usdg), 200, address(0), 0
        );
        vm.deal(address(this), 10 ether);
        escrow.credit{value: 10 ether}(address(d2));
        d2.harvest();
        assertEq(gv.balances(pid2, address(0)), 7 ether, "project leg deposited, attributed");
        assertEq(ops.balance, 3 ether, "ops leg unchanged");
        assertEq(gv.balances(PID, address(0)), 0, "the OTHER repo got nothing");
    }

    /* ------------------------------------------------------- platform leg -- */

    address constant PLATFORM = address(0x9137F0);

    function _withPlatform(uint16 platformBps) internal returns (OpenPonsDistributor d3) {
        d3 = new OpenPonsDistributor(
            IPonsFeeEscrow(address(escrow)), address(gv), PID, ops, 9_000,
            IPoolManager(address(pm)), address(usdg), 200, PLATFORM, platformBps
        );
    }

    function test_platformLegComesOffTheTopThenTheSplitApplies() public {
        OpenPonsDistributor d3 = _withPlatform(1_000); // 10%
        vm.deal(address(this), 10 ether);
        escrow.credit{value: 10 ether}(address(d3));
        d3.harvest();
        // 10% platform = 1 ETH ; du reste (9), 90% repo = 8.1, 10% ops = 0.9
        assertEq(PLATFORM.balance, 1 ether, "platform first, off the top");
        assertEq(gv.balances(PID, address(0)), 8.1 ether, "repo gets its share of the REST");
        assertEq(ops.balance, 0.9 ether, "ops gets the remainder of the rest");
        assertEq(address(d3).balance, 0, "nothing rests in the distributor");
        assertEq(d3.totalToPlatform(address(0)), 1 ether, "platform ledger kept");
    }

    function test_platformLegAppliesToTokensToo() public {
        OpenPonsDistributor d3 = _withPlatform(1_000);
        usdg.mint(address(escrow), 1_000e6);
        escrow.creditToken(address(d3), address(usdg), 1_000e6);
        d3.harvestToken(address(usdg));
        assertEq(usdg.balanceOf(PLATFORM), 100e6);
        assertEq(gv.balances(PID, address(usdg)), 810e6);
        assertEq(usdg.balanceOf(ops), 90e6);
    }

    function test_roundingDustStillFavoursTheRepo() public {
        OpenPonsDistributor d3 = _withPlatform(1_000);
        vm.deal(address(this), 1 ether);
        escrow.credit{value: 9_999}(address(d3));
        d3.harvest();
        // platform: 999 (rounded down). rest 9000: ops 900, repo 8100. Every wei accounted.
        assertEq(PLATFORM.balance, 999);
        assertEq(ops.balance, 900);
        assertEq(gv.balances(PID, address(0)), 8_100);
        assertEq(uint256(999 + 900 + 8_100), uint256(9_999), "no wei lost");
    }

    function test_zeroPlatformBpsIsExactlyTheOldBehaviour() public {
        OpenPonsDistributor d3 = _withPlatform(0);
        vm.deal(address(this), 10 ether);
        escrow.credit{value: 10 ether}(address(d3));
        d3.harvest();
        assertEq(PLATFORM.balance, 0);
        assertEq(gv.balances(PID, address(0)), 9 ether);
        assertEq(ops.balance, 1 ether);
    }

    function test_aHundredPercentRepoStillPaysThePlatform() public {
        OpenPonsDistributor d3 = new OpenPonsDistributor(
            IPonsFeeEscrow(address(escrow)), address(gv), PID, address(0), 10_000,
            IPoolManager(address(pm)), address(usdg), 200, PLATFORM, 1_000
        );
        vm.deal(address(this), 10 ether);
        escrow.credit{value: 10 ether}(address(d3));
        d3.harvest();
        assertEq(PLATFORM.balance, 1 ether);
        assertEq(gv.balances(PID, address(0)), 9 ether, "repo gets all of the rest");
    }

    function test_platformBpsIsCappedAtTwentyPercent() public {
        vm.expectRevert(OpenPonsDistributor.BadSplit.selector);
        new OpenPonsDistributor(
            IPonsFeeEscrow(address(escrow)), address(gv), PID, ops, 9_000,
            IPoolManager(address(pm)), address(usdg), 200, PLATFORM, 2_001
        );
    }

    function test_aPaidPlatformLegNeedsAnAddress() public {
        vm.expectRevert(OpenPonsDistributor.ZeroAddress.selector);
        new OpenPonsDistributor(
            IPonsFeeEscrow(address(escrow)), address(gv), PID, ops, 9_000,
            IPoolManager(address(pm)), address(usdg), 200, address(0), 1_000
        );
    }

    function test_aPayableRemainderNeedsAnOpsVault() public {
        vm.expectRevert(OpenPonsDistributor.ZeroAddress.selector);
        new OpenPonsDistributor(IPonsFeeEscrow(address(escrow)), address(gv), PID, address(0), 9_000,
            IPoolManager(address(pm)), address(usdg), 200, address(0), 0);
    }
}
