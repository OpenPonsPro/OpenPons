...
/**
 * Where a project launch's creator fees go, decided once and then not decidable again.
 *
 ...
 *
 * The failure mode this exists to avoid is not technical. Launchpads before this one routed
 * hundreds of millions in trading volume under project brands and credited the projects a
 * fraction of a percent, into wallets the projects did not control and mostly did not know
 * about, behind undisclosed fees. Nothing there was a hack.
 ...
 * point this contract's project share anywhere else. A donor can read that off the verified source
 ...
 *
 * ## ⭐⭐ THE PROJECT LEG NEVER LEAVES THE CHAIN
 *
 * The project's share is DEPOSITED into the shared `GitVault` under this launch's `projectId`, in
 * the same transaction as the harvest. No bridge, no custodial hop, no in-transit state: the money
 * is either in Pons's escrow, or attributed in the vault, or claimed. The one discretionary point
 * in the whole system is the claim verifier's key, and that lives in `GitVault`'s trust note, not
 * here.
 */
import {V4Seller, IPoolManager} from "./V4Seller.sol";
import {GitVault} from "./GitVault.sol";

contract OpenPonsDistributor is V4Seller {
    /* ------------------------------------------------------------------ config -- */
    ...
    /**
     * The shared per-repo treasury. The project leg is DEPOSITED there with its `projectId` —
     * never a bare transfer, or the vault receives anonymous money nobody can ever claim.
     */
    address public immutable gitVault;

    /// Which repo this launch funds. See `web/src/lib/projects.ts` for the encoding.
    ...
    /**
     * The protocol's cut, taken off the top of every release before the repo/launcher split.
     *
     * ⛔ Like every other term here: fixed at construction, no setter, the same for every launch
     * made through one launchpad, and printed on the launch form before anything is signed. A fee
     * that moves or hides is the exact failure this codebase exists to make impossible.
     */
    address public immutable platformVault;
    uint16 public immutable platformBps;

    /**
     * The project's share, in basis points of everything this contract receives.
     *
    ...
    event Released(address indexed asset, uint256 toProject, uint256 toOps);
    ...
    /**
     * @param projectBps_ The project's share. ⚠ Bounded at 10000 and NOT at some lower "sensible"
     *        number: 100% to project is the honest default for this contract, and a deployment that
    ...
    constructor(
        ...
        address platformVault_,
        uint16 platformBps_
    ) V4Seller(poolManager_, usdg_, maxSellSlippageBps_) {
        if (address(escrow_) == address(0) || gitVault_ == address(0)) revert ZeroAddress();
        // ⛔ A zero projectId would fund a treasury no verifier will ever sign for: money visibly
        // held, permanently unclaimable. The same class of mistake as stranding a stock.
        if (projectId_ == bytes32(0)) revert ZeroAddress();
        // ⚠ `opsVault` may only be zero when it can never be paid, or the remainder burns silently.
        if (opsVault_ == address(0) && projectBps_ != 10_000) revert ZeroAddress();
        if (projectBps_ > 10_000) revert BadSplit();
        // ⛔ 20% is already enormous for a protocol cut; past it, a typo at deploy time is the
        // only plausible explanation, and an immutable typo is forever.
        if (platformBps_ > 2_000) revert BadSplit();
        if (platformBps_ > 0 && platformVault_ == address(0)) revert ZeroAddress();
        escrow = escrow_;
        gitVault = gitVault_;
        projectId = projectId_;
        opsVault = opsVault_;
        projectBps = projectBps_;
     ...
     * Harvesting AAPL fees into `projectVault` moves them somewhere no bridge will take them, which
     * is not a delay, it is a permanent loss to the project. The launch itself must be paired against
    ...
    function harvestToken(address asset) public returns (uint256 gained) {
        ...
        /* ⭐⭐ A HARVESTED STOCK IS HELD, NOT PUSHED — and this is the whole fix for the stranding
           bug. Pushing AAPL to `projectVault` would move it somewhere no bridge will take it, which
     ...
     * sweep, harvest and payout can be run by a stranger, a cron, or the project itself, with no key
     ...
     *
     * ⚠ Public and permissionless so a stuck balance is recoverable to the project by anyone at all,
     * including the project, without this contract needing a rescue function that could do anything
     * else.
     */
    ...
    function _release(address asset) internal {
        /* ⛔⛔ THE PAYOUT ALLOWLIST, AND THE REASON IT IS A `revert` RATHER THAN A CONVENTION.
           ...
           for good — the project's money, visibly held, permanently unspendable. Making the payout
        ...
        /* ⭐ The platform leg comes OFF THE TOP, rounded down, and the repo/launcher split applies
           to the rest. Ordering matters for honesty: the site advertises the repo's share of the
           post-platform remainder, so the arithmetic here must be the arithmetic shown there. */
        uint256 toPlatform = (amount * platformBps) / 10_000;
        uint256 rest = amount - toPlatform;

        /* ⭐ The OPS side is the one that rounds down, so every wei of division dust lands on the
           project side. Backwards from the obvious `project = rest * bps / 10000`, deliberately:
           dust should never accumulate on the side holding the keys. */
        uint256 toOps = (rest * (10_000 - projectBps)) / 10_000;
        uint256 toProject = rest - toOps;

        totalToProject[asset] += toProject;
        totalToOps[asset] += toOps;
        totalToPlatform[asset] += toPlatform;
        emit Released(asset, toProject, toOps);

        /* ⚠ Project FIRST. If a push fails the whole call reverts and nothing is recorded, so the
           ordering only matters for gas — but it also means a reader of the trace sees the project
           leg settle before anything else can have touched the balance. */
        if (toProject != 0) {
            if (asset == address(0)) {
                GitVault(gitVault).deposit{value: toProject}(projectId);
            } else {
                /* Allowance granted and consumed in the same call; nothing lingers. */
                (bool okA,) = asset.call(abi.encodeWithSelector(0x095ea7b3, gitVault, toProject));
                if (!okA) revert TransferFailed();
                GitVault(gitVault).depositToken(projectId, asset, toProject);
            }
        }
        if (toOps != 0) _send(asset, opsVault, toOps);
        if (toPlatform != 0) _send(asset, platformVault, toPlatform);
    }
