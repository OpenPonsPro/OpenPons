...
import {OpenPonsDistributor, IPonsFeeEscrow} from "./OpenPonsDistributor.sol";
 ...
 * the project's distributor can be named at launch, so there is no `transferCreatorFeeRecipient`
...
/**
 * The launchpad: one transaction that creates a token whose fees can only ever reach a project.
 *
 * ## What one call does
 *
 * 1. Deploys a `OpenPonsDistributor` with the project address and the split written in as
 ...
 * an ordinary token wearing a project's name — the precise thing this exists to make impossible. A
...
contract OpenPonsLaunchpad {
    ...
    /**
     * The floor on a launch's project share, fixed at deployment.
     *
     * ⛔⛔ A LAUNCHPAD THAT ACCEPTS ANY SPLIT IS NOT A PROJECT LAUNCHPAD. Without a floor, the first
     * token to launch here at 1% would still carry the site's branding, the project's name and the
     * registry listing — the classic launchpad failure in one launch: a project's identity
     ...
     */
    uint16 public immutable minProjectBps;
    ...
    struct Entry {
        ...
        uint64 launchedAt;
        /**
         * ⭐⭐ WHICH REPO, WRITTEN IN FOREVER.
         *
         * The `owner/repo` slug itself, UTF-8 in a bytes32 (hashed only past 32 bytes — see
         * `web/src/lib/projects.ts`). Anyone can read it straight off an explorer: no directory,
         * no API, no us.
         *
         * ⛔⛔ THIS IS THE PROMISE, AND IT IS IMMUTABLE. Every distributor deposits this launch's
         * share into the shared `GitVault` under this id, and only a GitHub-verified admin of the
         * repo it names can ever take it out.
         */
        bytes32 projectId;
    }
    ...
    /// The shared per-repo treasury every distributor deposits into. One vault, forever.
    address public immutable gitVault;

    /**
     * The protocol's cut, inherited verbatim by every distributor this launchpad deploys.
     * ⛔ Immutable and printed on the launch form: the same fee for everyone, forever, or nothing.
     */
    ...
    constructor(
        ...
        uint16 minProjectBps_,
        address gitVault_,
        address platformVault_,
        uint16 platformBps_
    ) {
        if (address(factory_) == address(0) || usdg_ == address(0) || gitVault_ == address(0)) {
            revert ZeroAddress();
        }
        factory = factory_;
        ...
        minProjectBps = minProjectBps_;
        gitVault = gitVault_;
        // ⚠ The distributor's own constructor re-checks the cap and the zero-address pairing; a
        // bad pair here would fail the FIRST launch rather than this deploy, so check now too.
        if (platformBps_ > 2_000) revert ZeroAddress();
    ...
    /**
     * Launch a token whose creator fees are split between a project and the creator, permanently.
     *
     ...
     * @param projectId Which repo the fees fund: the `owner/repo` slug in a bytes32. ⛔ Recorded
     *        immutably and never validated on chain — a slug that names no real repository is a
     *        treasury nobody will ever be authorized to claim. The interface only mints it from a
     *        repo GitHub has confirmed exists.
     * @param creatorPayout Where the creator's remainder goes. May be zero only at a 100% split.
     * @param projectBps The project's share of every fee, for the life of the token.
     */
    struct DevBuy {
        uint256 quoteIn;
        uint256 minTokensOut;
    ...
    function launch(
        ...
        return _launch(
            ...
            ProjectTerms({projectId: projectId, creatorPayout: creatorPayout, projectBps: projectBps}),
     ...
     *
     * ➤ Here the fee recipient is a `OpenPonsDistributor`, a contract with no owner and no way to
     ...
     *
     * @param terms The project, its config id, the creator remainder and the split.
    ...
    function _launch(
        ...
        uint256 launchConfigId,
        address pairToken,
        ProjectTerms memory terms,
        DevBuy memory devBuy,
        address[] memory exemptions
    ) internal returns (address token, address curve, address distributor) {
        if (!factory.launchEnabled()) revert LaunchesClosed();
        if (terms.projectBps < minProjectBps) revert ProjectShareTooSmall(terms.projectBps, minProjectBps);
        // ⛔ A zero id would fund a treasury no verifier will ever sign for. Refused before deploy.
        if (terms.projectId == bytes32(0)) revert ZeroAddress();
        ...
        */
        ...
        _launches.push(
            Entry({
                ...
                creator: msg.sender,
                pairToken: pairToken,
                projectBps: terms.projectBps,
                launchedAt: uint64(block.timestamp),
                projectId: terms.projectId
            })
        );

        emit ProjectLaunch(token, distributor, terms.projectId, msg.sender, terms.projectBps);
    }
    ...
    /// that a caller would render as a project launch with a zero project.
    ...
    function isProjectLaunch(address token) external view returns (bool) {
