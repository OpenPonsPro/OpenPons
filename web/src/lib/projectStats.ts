...
/**
 * What THIS launchpad has paid each project, read from its own register.
 *
 * ## ⭐⭐ WHY THESE NUMBERS AND NOBODY ELSE'S
 *
 * An earlier version of this site showed a directory's third-party lifetime totals: a `$1.2m
 * raised` pill on each card and an `Over $2.9m raised` headline that summed the
 ...
 * actually paid toward that project. They start empty and they only ever grow from real launches.
 *
 * ## ⛔⛔ NEVER SUMMED ACROSS PAIR ASSETS
 *
 * A launch pays in whatever it is paired against, so one project can hold ETH from one launch and
 ...
 *
 * ⚠ "Paid" is the site's word for it everywhere, and it is the honest one: `totalToProject` is what
 * a distributor has PUSHED, not what is sitting swept and unclaimed. A repo's share lands in the
 * GitVault under its own slug, waiting for its maintainer, and the note under the grid says so
 * rather than the number pretending otherwise.
 */

export type ProjectStat = {
  /** Launches opened here that name this project. */
  launches: number
  /** pairToken (lowercased) -> paid, in that asset's own units. ⛔ Never added together. */
  byAsset: Record<string, bigint>
}

export type StatIndex = Record<string, ProjectStat>
...
export type StatRow = { projectId: string; pairToken: Address; paid: bigint }

/**
 * Fold the register into a per-project index.
 *
 ...
 */
export function indexByProject(rows: StatRow[]): StatIndex {
  const out: StatIndex = {}
  for (const r of rows) {
    const id = r.projectId?.toLowerCase()
    if (!id || id === ZERO_CONFIG) continue
    const stat = (out[id] ??= { launches: 0, byAsset: {} })
    stat.launches += 1
    const asset = r.pairToken.toLowerCase()
    /* ⚠ Counted even when zero, so a project with a launch that has not earned yet still shows the
...
export const statFor = (index: StatIndex, configId: string): ProjectStat | null =>
 ...
 *
 * ⛔ Returns a LIST, never a total. An asset that has paid nothing is dropped, so a project whose
 * only launch has not earned yet renders its launch count and no money line, rather than "0 ETH".
 */
export function paidParts(stat: ProjectStat): string[] {
...
/**
 * Whether ranking by amount is meaningful across a set of projects.
 *
...
export function paidIn(stat: ProjectStat | null, asset: string | null): bigint {
...
export function totals(index: StatIndex) {
  let launches = 0
  const projects = Object.keys(index).length
  const byAsset: Record<string, bigint> = {}
  for (const s of Object.values(index)) {
    launches += s.launches
    for (const [a, v] of Object.entries(s.byAsset)) byAsset[a] = (byAsset[a] ?? 0n) + v
  }
  return { launches, projects, paid: paidParts({ launches, byAsset }) }
}

/*
  ⚠ One in-flight request, shared, exactly like `loadProjects`. The directory page and the launch
  ...
  whose real job is choosing a project; an RPC hiccup must cost a launcher some figures on a card,
  not the ability to pick anybody at all.
*/
let statCache: StatIndex | null = null
let statInflight: Promise<StatIndex> | null = null

export function loadedProjectStats(): StatIndex | null {
  return statCache
}

export async function loadProjectStats(): Promise<StatIndex> {
  if (statCache) return statCache
  if (statInflight) return statInflight
  const { readProjectStatRows } = await import('./launchpad.ts')
  statInflight = readProjectStatRows()
    .then((rows) => (statCache = indexByProject(rows)))
    .catch(() => (statCache = {}))
